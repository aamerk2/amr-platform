...
interface Order {
  ...
  priority: string
  status: string
  items: { sku: string; qty: number; location: string }[]
}

export default function WMSPage() {
  const { orders, injectOrder, injectBatch } = useStore()

  const stats = {
    total:    orders.length,
    critical: orders.filter((o: Order) => o.priority === "CRITICAL").length,
