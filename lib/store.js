...
function makeTasks(order) {
  return order.items.map(item => ({
    ...
    from: [Math.floor(1 + Math.random() * (ROWS - 2)), Math.floor(1 + Math.random() * (COLS - 2))],
    ...
    completedAt: null,
    // G2P flow states:
    // WMS_QUEUED → WCS_DISPATCHED → AMR_ASSIGNED → AMR_MOVING →
    // AT_STATION → PICKING → PICKED → AMR_RETURNING → DONE
...
function makeAMR(index) {
  return {
    ...
    status: "IDLE",        // IDLE | MOVING_TO_RACK | MOVING_TO_STATION | AT_STATION | PICKING | RETURNING | CHARGING
...
export const useStore = create((set, get) => ({
  ...
  updateAMR: (amrId, updates) =>
    set(s => ({ amrs: s.amrs.map(a => a.id === amrId ? { ...a, ...updates } : a) })),

  // ── G2P AUTOMATION TICK ──
  // Called by RMS page every 500ms when running
  automationTick: () => {
    const { tasks, amrs, stations, addLog, orders } = get()

    // ── STEP 1: Auto WCS (process any still WMS_QUEUED) ──
    const hasQueued = tasks.some(t => t.status === "WMS_QUEUED")
    if (hasQueued) {
      set(s => ({
        tasks: s.tasks.map(t =>
          t.status === "WMS_QUEUED"
    ...
    // ── STEP 2: Assign WCS_DISPATCHED tasks to idle AMRs + free stations ──
    const freshTasks = get().tasks
    const freshStations = get().stations
    const freshAmrs = get().amrs

    const dispatched = freshTasks
      .filter(t => t.status === "WCS_DISPATCHED")
      .sort((a, b) => PRIORITY_SCORE[b.priority] - PRIORITY_SCORE[a.priority])

    const idleAmrs = freshAmrs.filter(a => a.status === "IDLE" && a.battery > 15)
    const freeStations = freshStations.filter(s => s.status === "IDLE")
    ...
    if (dispatched.length > 0 && idleAmrs.length > 0 && freeStations.length > 0) {
      ...
      // Assign task → AMR → Station
      set(s => ({
        tasks: s.tasks.map(t =>
          t.id === task.id
            ? { ...t, status: "AMR_ASSIGNED", g2pStatus: "AMR_ASSIGNED", assignedTo: amr.id, stationId: station.id, assignedStation: station.id }
            : t
        ),
        amrs: s.amrs.map(a =>
          a.id === amr.id
      ...
      addLog(`AMR ${amr.id} → RACK at [${task.from}] → ${station.label}`, "#e040fb", "RMS")
    }

    // ── STEP 3: Move AMRs ──
    const currentAmrs = get().amrs
    const currentTasks = get().tasks

    const updatedAmrs = currentAmrs.map(amr => {
      let { row, col, status, battery, targetRow, targetCol, path, totalDist, taskId, stationId, carryingRack, homeRow, homeCol } = amr
      ...
      if (battery < 8 && status !== "CHARGING") {
        ...
        if (stationId) {
          set(s => ({
            stations: s.stations.map(st => st.id === stationId ? { ...st, status: "IDLE", amrId: null, taskId: null } : st),
            tasks: taskId ? s.tasks.map(t => t.id === taskId ? { ...t, status: "WCS_DISPATCHED", g2pStatus: "WCS_DISPATCHED", assignedTo: null, stationId: null } : t) : s.tasks,
          }))
        }
        return { ...amr, battery, status: "CHARGING", taskId: null, stationId: null, carryingRack: false, targetRow: homeRow, targetCol: homeCol }
      }

      if (status === "CHARGING") {
        battery = Math.min(100, battery + 1.5)
        if (battery >= 95) return { ...amr, battery, status: "IDLE", row: homeRow, col: homeCol }
        return { ...amr, battery }
      }

      // Move toward target
      const move = (r: number, c: number, tr: number, tc: number) => {
        let nr = r, nc = c
        const inAisleCol = AISLE_COLS.includes(c)
        if (!inAisleCol) {
          const nearestAC = AISLE_COLS.reduce((a, b) => Math.abs(a - c) < Math.abs(b - c) ? a : b)
          nc = c + (nearestAC > c ? 1 : -1)
        } else if (r !== tr) {
          nr = r + (tr > r ? 1 : -1)
        } else if (c !== tc) {
          nc = c + (tc > c ? 1 : -1)
      ...
      if (status === "MOVING_TO_RACK") {
        if (atTarget || Math.random() < 0.025) {
          // Arrived at rack — pick it up, now move to station
          const task = currentTasks.find(t => t.id === taskId)
          const station = get().stations.find(s => s.id === stationId)
          if (!task || !station) return { ...amr, battery }

          // Station position (left side = col -1, right side = col COLS)
          const stRow = station.side === "left" ? [2, 7][parseInt(station.id.replace("S","")) - 1] || 2 : [2, 7][parseInt(station.id.replace("S","")) - 3] || 2
          const stCol = station.side === "left" ? 0 : COLS - 1
          ...
          set(s => ({
            ...
            stations: s.stations.map(st => st.id === stationId ? { ...st, status: "AMR_EN_ROUTE" } : st),
          }))
          ...
          return {
            ...
            targetRow: stRow,
            targetCol: AISLE_COLS[0],
            path: [...path, { row, col }].slice(-8),
            totalDist: totalDist + 1,
          }
        }
        const moved = move(row, col, targetRow, targetCol)
        return { ...amr, battery, ...moved, totalDist: totalDist + 1, path: [...path, moved].slice(-8) }
      }
      ...
      if (status === "MOVING_TO_STATION") {
        if (atTarget || Math.random() < 0.025) {
          ...
          set(s => ({
            ...
            stations: s.stations.map(st => st.id === stationId ? { ...st, status: "ACTIVE" } : st),
          }))
          return { ...amr, battery, status: "AT_STATION", row, col, path: [...path, { row, col }].slice(-8) }
        }
        const moved = move(row, col, targetRow, targetCol)
        return { ...amr, battery, ...moved, totalDist: totalDist + 1, path: [...path, moved].slice(-8) }
      }
      ...
      if (status === "AT_STATION") {
        // Wait at station — picking happens in Station page
        // Check if station says picking is done
        const station = get().stations.find(s => s.id === stationId)
        if (station?.status === "PICKING_DONE") {
          addLog(`${amr.id} returning rack to storage`, "#3b82f6", "RMS")
          const task = currentTasks.find(t => t.id === taskId)
          const homeRow = task?.from[0] || 2
          const homeCol = task?.from[1] || 4

          set(s => ({
            tasks: s.tasks.map(t => t.id === taskId ? { ...t, status: "AMR_RETURNING", g2pStatus: "AMR_RETURNING" } : t),
            stations: s.stations.map(st => st.id === stationId ? { ...st, status: "WAITING_RETURN" } : st),
          }))

          return { ...amr, battery, status: "RETURNING", targetRow: homeRow, targetCol: task?.from[1] || AISLE_COLS[0] }
        }
        return { ...amr, battery }
      }
      ...
      if (status === "RETURNING") {
        if (atTarget || Math.random() < 0.025) {
          ...
          set(s => ({
            ...
            stations: s.stations.map(st => st.id === stationId
            ...
            orders: s.orders.map(o => {
              ...
              return allDone ? { ...o, status: "SHIPPED" } : { ...o, status: "PROCESSING" }
            }),
          }))
  ...
  markPickingDone: (stationId) => {
    const { stations, tasks, addLog } = get()
    ...
    set(s => ({
      ...
      tasks: s.tasks.map(t =>
        t.id === station.taskId ? { ...t, status: "PICKED", g2pStatus: "PICKED", pickCompletedAt: Date.now() } : t
      ),
    }))
    addLog(`Picking complete at ${stationId} — AMR returning`, "#10b981", "STATION")
  },
